...
var mobileConsole = (function () {
    ...
    var setLineStyle = (function () {
            ...
            return function (element, type, msg) {
                if (status.initialized) {
                    color = (typeof msg === 'undefined' || msg === htmlToString(messages.empty)) ? {color: '#808080'} : ((msg === htmlToString(messages.clear)) ? {
                        color: '#808080',
                        fontStyle: 'italic'
                    } : (lineStyles(type) !== undefined ? lineStyles(type).text : lineStyles.log.text));
    ...
    function initConsole() {
        ...
        function urlFromString(string) {
            ...
            var match,
                uriPattern = /\b((?:[a-z][\w-]+:(?:\/{1,3}|[a-z0-9%])|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'".,<>?Â«Â»â€œâ€â€˜â€™]))/ig;
            try {
    ...
    function initCommandLine() {
        ...
        function getFromArrayById(id) {
            var pos = elements.acItems.map(function (x) {
                return x.id;
            }).indexOf(id);
            return {
                position: pos,
        ...
        function autoComplete(command) {
            ...
            var searchString = encodeURI(command), matches, match, row, i,
